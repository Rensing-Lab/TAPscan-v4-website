...
# will change the trees in _data/trees/
# original trees from Romy are backed up in _data/trees/originals
# to re-do the mapping, copy the original trees to _data/trees/ and run this script


# use the MadLand Zoo table for ID mapping, column 1 contains 5 letter code, column 9 contains NCBI taxid
genome_zoo="../../_data/MAdLand.Zoo.overview.table.tsv"

while IFS='|' read -r -a mappings
do
  code="${mappings[0]}"
  ncbi_taxid="${mappings[8]}"
  if [ -n "$ncbi_taxid" ];
  then
    echo "$code : $ncbi_taxid"
    sed -i "s/${code}/${ncbi_taxid}.${code}/" *.tre*
  fi
done < $genome_zoo
